...
async def main():
    ...
    with get_db_context() as db_session:
        ...
        logger = get_logger()
        logger.info(f'service started for card ids: {first_print_id}-{last_print_id} and tcp connection limit configured to {TCP_CONNECTION_LIMIT}')
        logger.info(f'there are {len(existing_cards)} cards already in the database and {len(errors_list)} previous errors logged')
